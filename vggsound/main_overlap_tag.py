...
def eval_overlap_tag(loader, device, args):
    ...
    tea_model.load_state_dict(torch.load('./results/teacher_mod_' + str(tea_type) + '_' + arch + '_' + str(args.num_frame) + '_overlap.pkl', map_location={"cuda:0": "cpu"}), strict=True)
    print(f'Finish Loading teacher model')

    net = ImageNet(args).to(device) if stu_type == 0 else AudioNet(args).to(device)

    tea = Tea(tea_type).cuda()
    stu = Stu(tea_type).cuda()
    ...
    acc = train_network_distill(stu_type, tea_model, args.num_epochs, loader, net, device, optimizer, args, stu, tea)
    return acc
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num-runs', type=int, default=2, help='num runs')
    ...
    parser.add_argument('--use_video_frames', type=float, default=3)
